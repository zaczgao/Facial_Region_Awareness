...
def load_farl(model_type, model_file) -> VisualTransformer:
    ...
    if os.path.isfile(model_file):
        ...
        print("=> loaded pre-trained model '{}'".format(model_file))
    else:
...
class FaRLClsWrapper(nn.Module):
    def __init__(self, bacobone, num_classes=1000):
        super().__init__()

        self.bacobone = bacobone
        self.fc = nn.Linear(768, num_classes)
        self.pool = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)), nn.Flatten(1))

    def forward(self, x):
        features, cls_tokens = self.bacobone(x)
        y = self.fc(cls_tokens[-1])
        # y = self.fc(self.pool(features[-1]))
...
if __name__ == '__main__':
    ...
    model_path = ""
    bacobone = FaRLVisualFeatures(model_type="base", model_path=model_path, forced_input_resolution=224)
    model = FaRLClsWrapper(bacobone)
    print(model)

    x = torch.randn(2, 3, 224, 224)
    out = model(x)

    calc_params(bacobone.visual)
